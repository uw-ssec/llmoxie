...
class SecretsManager:
    ...
    def create_postgres_admin_password(self) -> Output[str]:
        ...
        # No special characters to avoid issues with connection strings
    ...
    def create_adls_secrets(
        self,
        storage_account_name: Output[str],
        litellm_logs_container: str = "litellm-logs",
    ) -> None:
        """
        Store ADLS account name and container name in Key Vault for the AdlLogger.

        Args:
            storage_account_name: Storage account name
            litellm_logs_container: Container name for LiteLLM logs
        """
        create_secret(
            resource_group_name=self.resource_group_name,
            vault_name=self.vault_name,
            secret_name="adls-account-name",
            secret_value=storage_account_name,
            environment=self.environment,
            tags={
                "ManagedBy": "Pulumi",
                "Source": "AzureResource",
        ...
        create_secret(
            resource_group_name=self.resource_group_name,
            vault_name=self.vault_name,
            secret_name="adls-litellm-logs-container",
            secret_value=Output.from_input(litellm_logs_container),
            environment=self.environment,
            tags={
                "ManagedBy": "Pulumi",
                "Source": "AutoGenerated",
        ...
        pulumi.log.info("✓ Stored ADLS secrets (account name + container) in Key Vault")
