...
EMBEDDING_MODEL_NAME = (
    "sentence-transformers/all-MiniLM-L12-v2"  # Change this to try new models
)
...
def initialize_qdrant(
    documents, model_name: str, qdrant_path: Path, collection_name: str
):
    ...
    print(
        f"Creating new Qdrant collection '{collection_name}' with {len(documents)} documents using '{model_name}'."
    )
...
qdrant = initialize_qdrant(
    documents, EMBEDDING_MODEL_NAME, QDRANT_PATH, QDRANT_COLLECTION
)

# Check stored vectors
print(
    f"Number of vectors stored: {qdrant.client.count(collection_name=QDRANT_COLLECTION)}"
)
