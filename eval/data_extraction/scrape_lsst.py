...
def extract_topic_data(topic):
    return {
        ...
        "question_author": topic.get("posters")[0]["user_id"]
        if topic.get("posters")
        else None,
...
def main():
    ...
    if not qa_df.empty:
        ...
        qa_df.sort_values(
            by=["question_date", "answer_date"], ascending=[False, False], inplace=True
        )
        ...
        if os.path.exists(CSV_FILENAME):
            ...
            updated_df = pd.concat(
                [existing_df, qa_df], ignore_index=True
            ).drop_duplicates()
        else:
            updated_df = qa_df
